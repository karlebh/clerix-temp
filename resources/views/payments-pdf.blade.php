    ...
    <table class="min-w-full table-auto border-collapse shadow-md rounded-lg overflow-hidden">
        ...
        <tbody class="text-gray-600">
            @forelse ($payments as $payment)
                <tr>
                    <td class="px-6 py-4 border-t">{{ $payment->id }}</td>
                    <td class="px-6 py-4 border-t">{{ \Carbon\Carbon::parse($payment->date)->format('M d, Y') }}</td>
                    <td class="px-6 py-4 border-t">{{ $payment->amount }}</td>
                    <td class="px-6 py-4 border-t">{{ $payment->note }}</td>
                    <td class="px-6 py-4 border-t">{{ $payment->reason }}</td>
                    <td class="px-6 py-4 border-t">{{ $payment->created_at->format('Y-m-d') }}</td>
                </tr>
            @empty
                <tr>
